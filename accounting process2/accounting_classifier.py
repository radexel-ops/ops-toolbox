...
class DataTableFrame(ctk.CTkFrame):
    ...
    def __init__(self, master, on_row_select, on_cell_edit=None, **kwargs):
        # CTkFrame에서 지원하지 않는 파라미터 제거
        kwargs.pop('fg_color', None)
        super().__init__(master, fg_color="transparent", **kwargs)
        self.on_row_select = on_row_select
        self.on_cell_edit = on_cell_edit  # 셀 편집 완료 콜백
        self.df: Optional[pd.DataFrame] = None
        self.selected_index: Optional[int] = None
        self.item_ids: List[str] = []  # Treeview item IDs
        self.current_filter: str = "all"  # all, pending, classified, existing, needs_review
        self.column_mapping: Dict = {}  # 컬럼 매핑 정보
    ...
    def load_data(self, df: pd.DataFrame, column_mapping: Dict = None, highlighted_cells: set = None):
        ...
        Args:
            ...
            highlighted_cells: 음영 처리된 셀 좌표 set - {(row_idx, col_idx), ...}
        """
        # 기존 데이터 삭제
        ...
        # 셀 단위 음영 정보: {(row_idx, col_idx), ...}
        self.highlighted_cells = highlighted_cells or set()
        # 행 단위로도 체크할 수 있도록 row 인덱스 집합 생성
    ...
    def _refresh_display(self):
        """현재 필터에 따라 데이터 표시 갱신 (최적화 버전)"""
        # 기존 데이터 삭제 - detach 사용으로 최적화
        ...
        # 태그 색상 설정 (한 번만 실행)
        self.tree.tag_configure("highlighted", background="#4A1E4A")   # 음영표시: 보라색
        self.tree.tag_configure("verified", background="#1E3A1E")      # 검증완료: 녹색
        self.tree.tag_configure("ai_classified", background="#1E3A3A")  # AI분류: 청록색
        self.tree.tag_configure("existing", background="#3A3A1E")      # 기존분류: 황색
        self.tree.tag_configure("needs_review", background="#3A1E1E")  # 검토필요: 적색
        self.tree.tag_configure("normal", background="#25262B")        # 미분류: 기본
    ...
    def _on_double_click(self, event):
        ...
        # 컬럼 번호 파싱 (#0은 트리 컬럼, #1부터 데이터 컬럼)
        col_num = int(column_id.replace("#", ""))
        if col_num == 0:  # #0은 행 번호 컬럼, 편집 불가
            return

        # 컬럼 이름 가져오기
        columns = list(self.df.columns) if self.df is not None else []
        col_idx = col_num - 1  # #1 -> index 0
        if col_idx < 0 or col_idx >= len(columns):
            return

        column_name = columns[col_idx]

        # 편집 불가능한 컬럼 (시스템 관리 컬럼)
        readonly_columns = ["분류_상태"]  # 상태는 시스템이 관리
    ...
    def select_row(self, idx: int):
        ...
        # item_ids에서 해당 인덱스의 item_id 찾기
        if idx < len(self.item_ids) and self.item_ids[idx] is not None:
            item_id = self.item_ids[idx]
            self.tree.selection_set(item_id)
            self.tree.focus(item_id)
            self.tree.see(item_id)  # 스크롤해서 보이게
            self.selected_index = idx
...
class AIChatPanel(ctk.CTkFrame):
    def __init__(self, master, on_submit, on_reanalyze=None, on_file_analyze=None,
                 ...
                 on_approve_classification=None, on_remove_attachment=None, **kwargs):
        super().__init__(master, fg_color=SIDEBAR_COLOR, **kwargs)
        self.on_submit = on_submit
        self.on_reanalyze = on_reanalyze  # 재분석 콜백
        self.on_file_analyze = on_file_analyze  # 파일 분석 콜백
        self.on_keep_existing = on_keep_existing  # 기존 분류 유지 콜백
        self.on_manual_classify = on_manual_classify  # 직접 분류 입력 콜백
        self.on_reset_chat = on_reset_chat  # 대화 초기화 콜백
        self.on_save_attachment = on_save_attachment  # 첨부파일 저장 콜백
        self.on_get_attachments = on_get_attachments  # 첨부파일 목록 조회 콜백
        self.on_start_ai = on_start_ai  # AI 분석 시작 콜백
        self.on_approve_classification = on_approve_classification  # 분류 승인 콜백
        self.on_remove_attachment = on_remove_attachment  # 첨부파일 삭제 콜백
        self.current_row_index: Optional[int] = None
        self.is_complete = False
        self.ai_started = False  # AI 분석이 시작되었는지 여부
        self.user_inputs: List[str] = []  # 사용자 입력 기록
        self.attached_files: List[str] = []  # 첨부된 파일 경로
        self.linked_rows: List[int] = []  # 연관된 행 인덱스
        self.existing_classification: Optional[str] = None  # 기존 분류
        self.message_widgets: List[Dict] = []  # 메시지 위젯 추적 [{frame, sender, text, id}]
        self.message_counter: int = 0  # 메시지 ID 카운터
    ...
    def _setup_ui(self):
        ...
        # result_frame은 show_complete()에서 pack됨 (초기에는 숨김)
        ...
        self.no_files_label = ctk.CTkLabel(
            ...
            text="PDF, 이미지, Excel, Word 파일을 첨부하세요",
            font=(FONT_FAMILY, 9),
            text_color="gray"
        )
        ...
        self.no_linked_label = ctk.CTkLabel(
            ...
            text="계약금/잔금 등 연관된 거래가 있으면 연결하세요",
            font=(FONT_FAMILY, 9),
            text_color="gray"
        )
        ...
        self.manual_classify_btn = ctk.CTkButton(
            ...
            width=80,
            fg_color="#10B981",
            command=self._manual_classify
        )
        self.manual_classify_btn.pack(side="left")

        # 승인/거절 버튼 프레임 (AI 추천 시 표시)
    ...
    def set_row(self, idx: int, row_data: str, is_reanalyze: bool = False,
        ...
        self.is_complete = False
        self.ai_started = False  # AI 분석 시작 여부 초기화
        self.user_inputs = []  # 사용자 입력 초기화
        self.attached_files = []  # 첨부 파일 초기화
        self.linked_rows = []  # 연관 행 초기화
        self.existing_classification = existing_classification  # 기존 분류 저장
        self.message_widgets = []  # 메시지 위젯 초기화
        self.message_counter = 0  # 메시지 ID 카운터 초기화
        self.pending_classification = None  # 대기 중인 분류 초기화
        self.pending_reasoning = None  # 대기 중인 근거 초기화
    ...
    def show_initial_suggestions(self, suggestions: Dict):
        ...
        Args:
            suggestions: 분류 제안 정보
                - status: "confident" | "need_info" | "discretionary"
                - primary: 주요 계정과목 (확신있는 경우)
                - options: 선택 가능한 계정과목 목록
                - missing_info: 부족한 정보 목록 (need_info인 경우)
                - reason: 판단 근거
        ...
        if status == "confident":
            ...
            ctk.CTkLabel(
                suggest_frame,
                text="회사의 회계정책에 따라 선택 가능한 계정과목입니다:",
                font=(FONT_FAMILY, 10),
                text_color="white"
            ).pack(anchor="w", padx=10, pady=(5, 2))

            # 선택 가능한 옵션들
            options_frame = ctk.CTkFrame(suggest_frame, fg_color="#374151", corner_radius=5)
            ...
            # 부족한 정보 먼저 표시 (회계담당자가 무엇을 해야하는지 명확히)
            if missing_info:
                info_frame = ctk.CTkFrame(suggest_frame, fg_color="#7C2D12", corner_radius=5)
                ...
                for info in missing_info[:5]:  # 최대 5개
                    ctk.CTkLabel(
                        info_frame,
                        text=f"  • {info}",
                        font=(FONT_FAMILY, 10),
                ...
                ctk.CTkLabel(
                    info_frame,
                    text="위 정보를 입력하면 정확한 분류가 가능합니다.",
                    font=(FONT_FAMILY, 9),
                    text_color="#FDBA74"
                ).pack(anchor="w", padx=8, pady=(3, 5))

            # 가능한 분류 범위 (클릭 가능한 옵션으로 표시)
            if options:
                ctk.CTkLabel(
                    suggest_frame,
                    text="📋 현재 정보로 가능한 분류 (클릭하여 선택):",
                    font=(FONT_FAMILY, 10, "bold"),
                    text_color="white"
                ...
                for i, opt in enumerate(options[:5]):  # 최대 5개 표시
                    # 옵션에서 계정과목과 설명 추출
                    if isinstance(opt, str):
                        opt_account = opt
                        opt_desc = ""
                    elif isinstance(opt, dict):
                        opt_account = opt.get("account", opt.get("name", str(opt)))
                    ...
                    opt_btn = ctk.CTkButton(
                        ...
                        anchor="w",
                        height=24,
                        command=lambda acc=opt_account: self._quick_select_option(acc)
                    )
                ...
                ctk.CTkLabel(
                    suggest_frame,
                    text="💡 추가 정보를 입력하면 가능한 분류가 표시됩니다.",
                    font=(FONT_FAMILY, 10),
                    text_color="#9CA3AF"
                ).pack(anchor="w", padx=10, pady=5)
    ...
    def _attach_files(self):
        ...
        filetypes = [
            ("지원 파일", "*.pdf *.png *.jpg *.jpeg *.gif *.bmp *.tiff *.tif *.webp "
                        "*.xlsx *.xls *.csv *.docx *.doc *.hwp *.hwpx *.txt *.rtf "
                        "*.xml *.json *.yaml *.yml *.eml *.msg *.zip"),
            ("세금계산서/데이터", "*.xml *.json *.yaml *.yml"),
            ("PDF 문서", "*.pdf"),
            ("이미지", "*.png *.jpg *.jpeg *.gif *.bmp *.tiff *.tif *.webp"),
            ("스프레드시트", "*.xlsx *.xls *.csv"),
            ("문서", "*.docx *.doc *.hwp *.hwpx *.txt *.rtf"),
            ("이메일", "*.eml *.msg"),
            ("모든 파일", "*.*")
        ]
        ...
        if file_path in self.attached_files:
            ...
            if self.on_remove_attachment and self.current_row_index is not None:
                ...
                if success:
                    ...
                    messagebox.showwarning("삭제 실패", "첨부파일 삭제에 실패했습니다.")
            else:
                # 콜백이 없으면 메모리에서만 제거
                self.attached_files.remove(file_path)
                self._refresh_files_list()
    ...
    def _link_rows(self):
        ...
        if result:
            try:
                # 쉼표로 분리하고 정수로 변환
                rows = [int(r.strip()) - 1 for r in result.split(",")]  # 1-based to 0-based
                for r in rows:
    ...
    def _create_linked_chip(self, row_idx: int):
        ...
        remove_btn = ctk.CTkButton(
            ...
            fg_color="transparent",
            hover_color=ERROR_COLOR,
            command=lambda r=row_idx: self._remove_linked(r)
        )
        remove_btn.pack(side="right", padx=2)
    ...
    def _analyze_files(self):
        """첨부 파일 분석 요청"""
        if not self.attached_files:
            messagebox.showinfo("알림", "분석할 파일을 먼저 첨부하세요")
            return

        if self.on_file_analyze and self.current_row_index is not None:
            self.add_message("System", f"{len(self.attached_files)}개 파일 분석 중...")
            self.on_file_analyze(self.current_row_index, self.attached_files)
    ...
    def add_message(self, sender: str, text: str, is_question: bool = False):
        """대화 메시지 추가 (텍스트 선택 및 복사 가능, 삭제 버튼 포함)"""
        if sender == "AI":
            bg = "#2D3748"
            text_color = "white"
        elif sender == "User":
        ...
        # 텍스트 영역 (선택 가능한 Text 위젯 사용)
        # 줄 수 계산 - 텍스트 전체가 보이도록 높이 자동 조정
        text_widget = tk.Text(
            msg_frame,
            font=(FONT_FAMILY, 11),
            fg=text_color,
        ...
        text_widget.configure(state="disabled")  # 편집 불가, 선택은 가능
        text_widget.pack(fill="x", padx=10, pady=(2, 8))

        # 텍스트 전체가 보이도록 높이 자동 조정
        def adjust_height(event=None):
            ...
            if widget_width > 1:  # 위젯이 렌더링된 후
                font = tk_font.Font(font=text_widget.cget("font"))
                avg_char_width = font.measure("가")  # 한글 기준
                chars_per_line = max(widget_width // avg_char_width, 1)

                total_lines = 0
                for line in text.split('\n'):
                    line_len = len(line)
                    total_lines += max(1, (line_len + chars_per_line - 1) // chars_per_line)
                ...
                lines = text.count('\n') + 1
                chars = len(text)
                estimated = max(lines, chars // 40 + 1)
                text_widget.configure(height=estimated)
    ...
    def show_complete(self, classification: str, reasoning: str):
        ...
        # 결과 표시 (이미 pack되어 있으면 먼저 제거)
        self.result_frame.pack_forget()
        self.result_label.configure(text=f"최종 분류: {classification}")
        # message_frame 위에 표시하기 위해 message_frame을 재배치
    ...
    def _manual_classify(self):
        ...
        # 직접 입력 다이얼로그
        dialog = ManualClassifyDialog(self)
        if dialog.result:
    ...
    def show_loading(self, loading: bool):
        ...
        if loading:
            ...
            self.start_ai_btn.configure(state="disabled", text="분석 시작됨")  # AI 시작 후에는 비활성화 유지
...
{{
    ...
    "options": [
        {{"account": "계정과목 코드", "description": "설명"}},
        ...
    ],
...
status 설명:
- "confident": 정보가 충분하여 확신있게 분류 가능
- "need_info": 추가 정보 필요 (missing_info에 명시)
- "discretionary": 회사 재량으로 선택 가능한 항목들 존재"""
    ...
    def _handle_ai_result(self, sheet_name: str, row_idx: int, result: Dict):
        ...
        if status == "COMPLETE":
            ...
            if is_current_row:
                # 분류 추천 표시 (승인 버튼과 함께)
                self.chat_panel.show_classification_recommendation(final_cls, reasoning)
            else:
