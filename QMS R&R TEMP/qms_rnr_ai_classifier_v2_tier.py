...
FILE_CONFIG = {
    "TARGET_FILE": "QMS_RnR_최종결과_v5_직무구분.xlsx",  # 입력 겸 출력 파일
...
def run_review_and_fix(client, model_config: dict, df: pd.DataFrame, context: dict) -> tuple:
    """
    [수정됨] 직무 수준(L1~L5) 분류 전용 함수
    기존 내용을 기반으로 각 행의 '직무 수준'을 5단계로 분류하여 업데이트합니다.
    """
    print("\n[Job Level Classifier] 직무 수준 분석 데이터 생성 중...")

    # 1. 분석 대상 텍스트 생성 (미래 담당자/직책 위주로 구성)
    lines = ["## [분석 대상 직무 목록]", ""]
    for idx, row in df.iterrows():
        job = str(row.get('QMS 직무', '')).strip()
        desc = str(row.get('설명', '')).strip()
        # 미래 담당자 정보가 판단의 핵심 기준임
        future_team = str(row.get('미래 담당팀', '')).strip()
        future_role = str(row.get('미래 담당자/직책', '')).strip()
        future_reason = str(row.get('미래 배정 이유', '')).strip()

        lines.append(f"[{idx}] QMS 직무: {job}")
        lines.append(f"    업무 설명: {desc}")
        lines.append(f"    미래 담당팀: {future_team}")
        lines.append(f"    미래 담당자/직책: {future_role}")
        lines.append(f"    미래 배정 이유: {future_reason}")
        lines.append("")

    target_text = "\n".join(lines)

    # 2. 프롬프트 구성 (5단계 레벨 가이드라인 포함)
    prompt = f"""
    당신은 조직 설계 전문가입니다. 아래 제공된 [분석 대상 직무 목록]을 보고, 각 직무의 책임 범위와 권한, 그리고 '미래 담당자/직책'을 고려하여 **'직무 수준'**을 다음 5단계 중 하나로 분류하세요.

    **[직무 수준 분류 기준]**
    1. **L5 (임원 / C-Level)**
       - 역할: 전략 수립, 법적/사업적 최종 책임(Accountability), 리스크 수용 여부 결정
       - 권한: 최종 승인 (Approve)
       - 키워드: C-Level, 본부장, 총괄, 최종 승인, 품질책임자(법적)

    2. **L4 (디렉터 / 부서장)**
       - 역할: 조직 관리(Management), 여러 팀 총괄, 자원 배분, 성과 관리
       - 권한: 승인 (Approve) 또는 상위 검토
       - 키워드: 실장, 센터장, 부서장, 디렉터, Director

    3. **L3 (팀 리드 / 파트장)**
       - 역할: 실무 리딩(Leading), 운영/일정 관리, 팀원 멘토링
       - 권한: 1차 검토 (Review)
       - 키워드: 팀장, 파트장, 리드, Lead, Manager (관리직 성격)

    4. **L2 (시니어 / 스페셜리스트)**
       - 역할: 독자적 전문 수행(Execution), 문제 해결, 주니어 코칭, 복잡한 실무
       - 권한: 작성 (Draft), 상호 검토, 실무 책임
       - 키워드: 선임, 책임, 수석, Specialist, 과장/차장급, 전문 담당자, Engineer (숙련)

    5. **L1 (주니어 / 어소시에이트)**
       - 역할: 가이드 하에 업무 수행, 단순/반복 실무, 지원(Support)
       - 권한: 작성 (Draft)
       - 키워드: 사원, 주니어, 보조, 지원, Associate, 담당자 (단순)

    **[작업 지시]**
    - 제공된 목록의 모든 항목(idx)에 대해 빠짐없이 분류하세요.
    - '미래 담당자/직책'에 적힌 내용을 최우선으로 고려하되, 직무 설명의 난이도도 참고하세요.
    ...
    **[출력 형식]**
    ...
    # 4. 결과 적용 (직무 수준 컬럼 업데이트)
    modified_count = 0
    modified_indices = []

    # '직무 수준' 컬럼이 없으면 생성
    if '직무 수준' not in df.columns:
        df['직무 수준'] = ''
        print("[System] '직무 수준' 컬럼이 생성되었습니다.")

    print(f"\n[Update] 분석된 {len(results)}건의 데이터를 적용합니다...")
    ...
    for item in results:
        ...
        # 기존 값과 다를 경우에만 업데이트 (또는 비어있을 때)
        current_val = str(df.at[idx, '직무 수준']).strip()
        if current_val != level:
            df.at[idx, '직무 수준'] = level
            modified_count += 1
